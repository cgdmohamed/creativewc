...
@Component({
  ...
  standalone: true,
  imports: [IonicModule, CommonModule, FormsModule, RouterModule, ComponentsModule, TranslateModule, LanguageSelectorComponent],
  template: `
    <ion-header>
    ...
    <ion-content>
      <div class="ion-padding">
        <h2>{{ 'SETTINGS.APP_SETTINGS' | translate }}</h2>
        
        <!-- Language Selector -->
        <ion-card>
          <ion-card-header>
            <ion-card-title>{{ 'SETTINGS.LANGUAGE' | translate }}</ion-card-title>
            <ion-card-subtitle>{{ 'SETTINGS.LANGUAGE_DESC' | translate }}</ion-card-subtitle>
          </ion-card-header>
          <ion-card-content>
        ...
        <ion-card *ngIf="isLoggedIn">
          <ion-card-header>
            <ion-card-title>{{ 'SETTINGS.USER_INFO' | translate }}</ion-card-title>
          </ion-card-header>
          <ion-card-content>
            <ion-item lines="none">
              <ion-label>
                <h2>{{ userData?.display_name || ('SETTINGS.DEFAULT_USER' | translate) }}</h2>
                <p>{{ userData?.email || ('SETTINGS.NO_EMAIL' | translate) }}</p>
              </ion-label>
            </ion-item>
          </ion-card-content>
        </ion-card>
        ...
        <ion-card>
          <ion-card-header>
            <ion-card-title>{{ 'SETTINGS.NOTIFICATIONS' | translate }}</ion-card-title>
            <ion-card-subtitle>{{ 'SETTINGS.NOTIFICATIONS_DESC' | translate }}</ion-card-subtitle>
          </ion-card-header>

          <ion-card-content>
            <ion-list lines="full">
              <ion-item>
                <ion-label>{{ 'SETTINGS.ENABLE_NOTIFICATIONS' | translate }}</ion-label>
                <ion-toggle [(ngModel)]="notificationsEnabled" (ionChange)="toggleNotifications($event)"></ion-toggle>
              </ion-item>
              
              <ion-item *ngIf="notificationsEnabled">
                <ion-label>{{ 'SETTINGS.ORDER_NOTIFICATIONS' | translate }}</ion-label>
                <ion-toggle [(ngModel)]="orderNotificationsEnabled" (ionChange)="updateNotificationCategories()"></ion-toggle>
              </ion-item>
              
              <ion-item *ngIf="notificationsEnabled">
                <ion-label>{{ 'SETTINGS.PROMOTION_NOTIFICATIONS' | translate }}</ion-label>
            ...
            <div class="ion-padding">
              <ion-button expand="block" (click)="refreshNotificationToken()" [disabled]="!notificationsEnabled">
                {{ 'SETTINGS.REFRESH_TOKEN' | translate }}
              </ion-button>
            </div>
            
            <div *ngIf="playerId" class="ion-text-center ion-padding-top">
              <small>{{ 'SETTINGS.DEVICE_ID' | translate }}: {{ playerId }}</small>
            </div>
          </ion-card-content>
        </ion-card>
        ...
        <ion-card>
          <ion-card-header>
            <ion-card-title>{{ 'SETTINGS.GENERAL_SETTINGS' | translate }}</ion-card-title>
          </ion-card-header>
          <ion-card-content>
            <ion-list lines="full">
              <ion-item>
              ...
              <ion-item>
                <ion-label>{{ 'SETTINGS.DATA_SAVING' | translate }}</ion-label>
        ...
        <ion-card>
          <ion-card-header>
            <ion-card-title>{{ 'SETTINGS.APP_INFO' | translate }}</ion-card-title>
          </ion-card-header>
          <ion-card-content>
            <ion-item lines="none">
              <ion-label>
                <h2>{{ 'SETTINGS.APP_VERSION' | translate }}</h2>
                <p>1.0.0</p>
              </ion-label>
            </ion-item>
          </ion-card-content>
